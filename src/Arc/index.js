...
const ArcAnimation = () => {
  ...
  const easeInOutCubic = (t) =>
    t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2; //Animation for ease in out for cubic

  const moveToPoint = (targetProgress) => {
    //for moving ball from point to point
    const arcPath = arcPathRef.current;
    const length = arcPath.getTotalLength();
    const animationDuration = 8000; // Time for travelling for point to point
    ...
    const animate = (currentTime) => {
      ...
      const newProgress =
        currentProgress + (targetProgress - currentProgress) * easedProgress;
      ...
      if (ballRef.current) {
        ballRef.current.style.transform = `translate(${
          (coords.x / 100) * containerWidth - 7.5
        }px, ${(coords.y / 50) * containerHeight - 7.5}px)`;
      }
